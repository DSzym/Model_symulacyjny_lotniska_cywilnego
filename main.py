...
class Plane:
    def __init__(self, time_arrival: int, time_departure: int, status: str, plane_id: int,
        ...
        self.time_to_prepare_for_departure = time_difference(self.arrival,
                                                             self.departure)  # nawet jeśli samolot wylądował później, to nie może wylecieć szybciej niż zaplanowany czas jego obsługi
        print(
            f"Class PLane initialization parameters: id: {self.id}{nl}arrival: {self.arrival}{nl}departure: {self.departure}{nl}actual arrival: {self.actual_arrival}{nl}actual departure: {self.actual_departure}{nl}emergency?: {self.if_emergency}{nl}service time: {self.time_to_prepare_for_departure}{nl} ")
...
class Schedule:
    ...
    def generate_schedule(self, nr_of_planes, prob_of_emergency):
        ...
        for i in range(nr_of_planes):
            departure_hour = np.random.uniform(arrivals_hour[i] + 1, arrivals_hour[i] + 4, 1).astype(
                int)  # ToDo: można dać więcej niż simulation_duration, to będzie bardziej rzeczywiste
            departure_minutes = 0
    ...
    def print_data(self):
        for plane in self.list_of_planes:
            print(
                f"Class PLane initialization parameters: id: {plane.id}{nl}arrival: {plane.arrival}{nl}departure: {plane.departure}{nl}actual arrival: {plane.actual_arrival}{nl}actual departure: {plane.actual_departure}{nl}emergency?: {plane.if_emergency}{nl}service time: {plane.time_to_prepare_for_departure}{nl} ")
...
class Airport:
    ...
    def __init__(self, nr_of_runways, max_nr_of_planes_on_ground):
        ...
        self.planes_currently_on_the_airport = []  # wyłącznie te, które są obecnie obsługiwane, bez tych na pasie, w kolejce do odlotu
        self.list_runways_occupation = []

    def initialize_runway_occupation(self):
        for i in range(self.number_of_runways):
            self.runways_occupation[i + 1] = [0, '', '', 0]  # [czas, id_samolotu, a - lądowanie, d- odlot, licznik samolotów]
        print(f"Initialized runway occupation: {nl}{self.runways_occupation}{nl}")

    def check_runways_occupation(self):
        # print(f"Checking runway occupation: {nl}{self.runways_occupation}{nl}")
    ...
    def new_plane_on_runway(self, runway_nr, plane, reason, current_time):
        # nie każdy samolot musi mieć ten sam czas lądowania bądź startu
        # print(f"Assign plane {plane} to runway {runway_nr} for {reason} {nl}")
        # print(f"Checking runway occupation before plane added: {nl}{self.runways_occupation}{nl}")
        ...
        # print(f"Checking runway occupation after plane added: {nl}{self.runways_occupation}{nl}")
...
current_time = 0
probability_of_emergency = 0.1  # = input("Podaj prawdopodobieństwo, że samolot otrzyma pierwszeństwo obsługi podczas przylotu: ")
simulation_duration = 2400  # format HHMM  #ToDo: uwzględnienie długości symulacji w generacji schedule
number_of_planes = 15  # = input("Podaj ")
liczba_pasow_startowych = 2 #input("Podaj liczbę pasów startowych na lotnisku: ")
maks_samolotow_na_ziemi = 5 #input("Podaj maksymalną liczbę samolotów, która może w tym danej chwili przebywać na "
                                #"lotnisku: ")
...
while current_time < simulation_duration:
    ...
    for runway_nr in simulation_Airport.runways_occupation.keys():  # self.runways_occupation[i+1] = [0, '', '']  # [czas, samolot, a - lądowanie, d- odlot]
...
# ToDo: Wykres opóźnień samolotów
...
# ToDo: zaznaczenie na osi czasu kiedy jaki samolot przyleciał i wyleciał
...
# ToDo: wykres użycia pasów startowych
...
for runway_nr in simulation_Airport.runways_occupation.keys():  # self.runways_occupation[i+1] = [0, '', '']  # [czas, samolot, a - lądowanie, d- odlot]
    ...
    bars_colors = []
    for registered_flight in simulation_Airport.list_runways_occupation:  # [runway_nr, current_time, time_to_leave_runway, plane, reason]
...
ax.set_xlim(0, 2500)
ax.set_xlabel('time')
ax.set_yticks(y_labels_points, labels=y_labels)  # zależy od liczby pasów startowych
